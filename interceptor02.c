...
asmlinkage long interceptor(struct pt_regs reg) {
	...
	// get the ax argument as the original syscall
	int syscall = reg.ax;
...
asmlinkage long my_syscall(int cmd, int syscall, int pid) {
	...
	if(cmd <0 || syscall < 0 || syscall > NR_syscalls || syscall == MY_CUSTOM_SYSCALL || pid<0){
		return -EINVAL;
	}
	...
	if(cmd == REQUEST_START_MONITORING){
		...
		// check valid syscall MUST BE INTERCEPT 
		...
		// also, the pid should not be monitored already
		...
		if(pid == 0){
			...
			// check if the pid is not in monitoring
...
/**
* helper function that check all the 3 argument passed into my_syscall
* error code (-EINVAL) if any of the argument are not valid.
*/

// int check_arg(int cmd, int syscall, int pid){
// 	// check a)
// 	if(cmd <0 || syscall < 0 || pid < 0 || syscall > NR_syscalls || syscall == MY_CUSTOM_SYSCALL){
// 		return -EINVAL;
// 	}
// 	// b) will check with the actuall cmd
// }
